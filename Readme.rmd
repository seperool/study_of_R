...
- Transformando um **data.frame** em tipo **tibble**, através da função **as_tibble()**.\
`x <- as_tibble(x)`\

- Verificando se uma tabela é tipo **tibble**, através da função **is_tibble()**. Retorna **TRUE** (se verdadeiro), ou **FALSE** (se falso).\
`is_tibble(x)`\
